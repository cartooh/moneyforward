...
## エッジケーステスト
- **ユニークインデックス引数なし**: unique_index_labelがNoneの場合、新規行として追加。
- **ユニークインデックス列欠損**: unique_index_labelが指定されているが、Excelファイルにその列が存在しない場合、新規行として追加。
...
- **シート不存在**: 指定シートがない場合、新規作成。 (テスト未実施: 新規ファイル作成テストでカバー済み)